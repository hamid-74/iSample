...
for i in range(no_of_clients):
  bandwidths.append(random.uniform(2,8))

with open('bandwidths' + '.txt', 'w') as filehandle: 
    for listitem in bandwidths:
      filehandle.write('%s\n' % listitem)
...
for k in range(no_of_configurations):
  ...
  with open('selective_results' + str(k) + '.txt', 'w') as filehandle: 
    for listitem in global_accuracy:
      filehandle.write('%s\n' % listitem)

  with open('epoch_times' + str(k) + '.txt', 'w') as filehandle: 
    for listitem in epoch_time:
      filehandle.write('%s\n' % listitem)

  with open('coefficients' + '.txt', 'w') as filehandle: 
    for listitem in coefficients:
      filehandle.write('%s\n' % listitem)
