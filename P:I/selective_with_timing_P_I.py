...
for k in range(len(coefficients)):
  ...
  for i in range(epochs):
    ...
    if i%(int(epochs/4)) == 0:
      ...
      plt.savefig('cnn_epoch' + str(i+1) + '_cfg' + str(k+1) + 'P_I.png', dpi = 600)
      plt.close()
  ...
  with open('selective_results_cnn_P_I' + str(k) + '.txt', 'w') as filehandle: 
    for listitem in global_accuracy:
      filehandle.write('%s\n' % listitem)

  with open('epoch_times_cnn_P_I' + str(k) + '.txt', 'w') as filehandle: 
