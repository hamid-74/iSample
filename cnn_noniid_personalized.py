...
sending_global_models_time = 0.405 #different for different models
...
def create_indexes(clients_training_images, clients_training_labels, no_of_clients):
  ...
  for i in range(no_of_clients): 
    ...
    with open('non_iid_indexes/index' + str(i+1) + '.txt', 'r') as filehandle:
      for line in filehandle:
        current_number = line[:-1]
...
for k in range(len(coefficients)):
  ...
  for i in range(epochs):
    ...
    for j in range (int(no_of_clients/4)):
      ...
      if (bandwidths[sorted_grades[j][1]] + latencies[(sorted_grades[j][1])%4]) > max_comm_time:
        max_comm_time = bandwidths[sorted_grades[j][1]] + latencies[(sorted_grades[j][1])%4]
    ...
    if i%(int(epochs/4)) == 0:
      ...
      plt.savefig('cnn_epoch' + str(i+1) + '_cfg' + str(k+1) + '_non_iid_personalized_.png', dpi = 600)
      plt.close()
    ...
    for j in range(no_of_clients):
      ...
      clients[j].set_weights(new_weights)


    temp_epoch_time = training_time + receive_parameters_time + sending_request_time + max_comm_time + averaging_time(int(no_of_clients/4)) + sending_global_models_time + averaging_time(2)
    epoch_time[i] = temp_epoch_time
    print("temp_epoch_time", temp_epoch_time)


  with open('selective_results_cnn_non_iid_personalized_' + str(k) + '.txt', 'w') as filehandle: 
    for listitem in global_accuracy:
      filehandle.write('%s\n' % listitem)

  with open('epoch_times_cnn_non_iid_personalized_' + str(k) + '.txt', 'w') as filehandle: 
