...
for k in range(2,3):
  ...
  for i in range(no_of_clients):
    ...
    clients[i].inputs = inputs
    clients[i].outputs = classification_output
    ...
    clients[i].compile(optimizer='SGD', loss='sparse_categorical_crossentropy', metrics = ['accuracy'])
