...
clients_training_images_temp = list()
clients_training_labels_temp = list()


create_indexes(clients_training_images_temp, clients_training_labels_temp, no_of_clients)


clients_training_datasets = list()


for i in range(no_of_clients):
  train_ds_temp = tf.data.Dataset.from_tensor_slices((clients_training_images_temp[i],clients_training_labels_temp[i]))
  train_ds_size_temp =tf.data.experimental.cardinality(train_ds_temp).numpy()
  train_ds_temp=(train_ds_temp
          .map(process_image)
          .shuffle(buffer_size=train_ds_size)
          .batch(batch_size=32,drop_remainder=True)
         )
  clients_training_datasets.append(train_ds_temp)
...
for k in range(2,3):
  ...
  for i in range(no_of_clients):
    ...
    clients.append(tf.keras.applications.efficientnet.EfficientNetB0())
  ...
  for i in range(epochs):
    ...
    for j in range(no_of_clients):
      ...
      clients[j].fit(clients_training_datasets[j], epochs=1, validation_data=test_ds)
      
      clients_loss[j][i], clients_accuracy[j][i] = clients[j].evaluate(test_ds, verbose=2)
    ...
    global_loss[i], global_accuracy[i] = global_model.evaluate(test_ds, verbose=2)
