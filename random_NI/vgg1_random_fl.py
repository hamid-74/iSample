...
for k in range(no_of_configurations):
  ...
  for i in range(epochs):
    ...
    for j in range (int(no_of_clients/4)):
      ...
      if (bandwidths[random_indexes[j]-1] * size_factor + latencies[(random_indexes[j]-1)%4]) > max_comm_time:
        max_comm_time = bandwidths[random_indexes[j]-1] * size_factor + latencies[(random_indexes[j]-1)%4]
  ...
  with open('vgg1_random_results' + '.txt', 'w') as filehandle: 
    for listitem in global_accuracy:
      filehandle.write('%s\n' % listitem)

  with open('vgg1_random_epoch_times'  + '.txt', 'w') as filehandle: 
